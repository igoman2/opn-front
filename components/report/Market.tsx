        ...
        <>
            ...
            <Body>
                <Card>
                    ...
                    {input.market_max_month}) 이며, 최소값은 월{" "}
                    {input.market_min_size} ({input.market_min_year}{" "}
                    {input.market_min_month}) 입니다. 최근 3년간 시장규모 추세는
                    월 {input["3y_trend_start_year"]}{" "}
                    {input["3y_trend_start_market_size"]}에서 월{" "}
                    {input["3y_trend_end_year"]}{" "}
                    {input["3y_trend_end_market_size"]} 으로{" "}
                    {input["3y_trend_percent"]} {input.market_trend}하였습니다.{" "}
                    {input.address_dong}의 현재 {input.department} 의원 개수는{" "}
                    {input.hospital_count_now}입니다. 최근 3년간 의원 개수
                    추세는 {input["3y_trend_start_year"]}{" "}
                    {input.hospital_count_past}에서 {input["3y_trend_end_year"]}{" "}
                    {input.hospital_count_now}로{" "}
                    {input["3y_trend_hospital_count_difference"]}{" "}
                    {input.hospital_count_trend}하였습니다.<br></br>
                    평수가 큰 의원일수록 매출이 높은 경향이 있기 때문에, 평수의
                    효과를 제거한 1평당 매출액을 확인하는 것이 중요합니다. 의원
                    1평당 매출액은 월 {input.profit_per_area_size}으로, 50평
                    의원 기준으로 환산하면 월 {input.profit_per_50p} 입니다.
                    최근 3년 추세는{" "}
